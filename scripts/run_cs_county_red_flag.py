"""County-grain Callaway-Sant'Anna ATT(g, t) for civil-petition red-flag
laws (extreme risk protection orders).

Treatment: a state's first 0->1 switch in the Tufts `law_gvro` indicator
(allows civilian petition for an extreme risk protection order). The
older law-enforcement-only ERPOs are NOT picked up here. Treatment is
treated as absorbing.

This is the county-grain parallel of `scripts/run_cs_red_flag.py`.
The county panel does not carry `law_gvrolawenforcement` directly so
the strict control rule here is the simpler `law_gvro == 0`. (The
state-grain version requires both `gvro==0` AND `gvrolawenforcement==0`
throughout [g-5, g+5]; that extra condition cannot be enforced at the
county-panel layer because the county build only carries the
civil-petition variable.)
...
Specs (4): {OR, RA} x {broad, strict}.

Outputs:
  outputs/red_flag_cs_county/{att_gt,event_study,overall_att,
                              cohort_n,dropped_log}.csv
  outputs/red_flag_cs_county/figures/event_study_{control_rule}_{spec}_4panel.svg
"""
...
OUT = ROOT / "outputs" / "red_flag_cs_county"
FIG = OUT / "figures"
...
# Strict control: must have law_gvro == 0 throughout [g-5, g+5]. (See
# module docstring re: gvrolawenforcement not being on the county panel.)
STRICT_RULE_VARS = ("law_gvro",)
...
def main():
    ...
    print("Deriving GVRO cohorts (treatment = first 0->1 switch in law_gvro) ...")
    cohorts, never_treated, dropped = derive_state_cohorts_for_county(
        panel, treatment_var="law_gvro", direction="0to1",
    ...
    for control_rule in ("broad", "strict"):
        for spec in ("or", "ra"):
            ...
            plot_event_study_county(
                ...
                title_suffix="-- red-flag (law_gvro)")
            print(f"  Wrote "
